...
urllib.request.urlretrieve(
    "https://github.com/imjoy-team/imjoy-interactive-segmentation/releases/download/v0.1.0/test_samples_4.pkl",
    "test_samples_4.pkl",
)
...
test_mask = model.predict(test_image, channels=(1, 2), diameter=100.0)
package = model.export(
    "bioimageio",
    os.path.join(os.path.dirname(model.default_save_path), "bioimageio.zip"),
    test_image,
    test_mask,
)
with open("cellpose_weights.zip", "wb") as fil:
    fil.write(package)
...
# Create a combined cover image
# test_image = image.astype('uint8')
# test_mask = mask_rgb(labels)
# combined = np.concatenate([test_image, test_mask], axis=1)
# imageio.imwrite(f"cover.png", combined.astype("uint8"))
